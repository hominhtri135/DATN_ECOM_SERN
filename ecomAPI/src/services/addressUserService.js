...
const createNewAddressUser = (data) => {
...
const getAllAddressUserByUserId = (userId) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!userId) {
        resolve({
        ...
        const res = await db.AddressUser.findAll({
          where: { userId: userId },
        });
        resolve({
          errCode: 0,
...
const getDetailAddressUserById = (id) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!id) {
        resolve({
        ...
        const res = await db.AddressUser.findOne({
          where: { id: id },
        });
        resolve({
          errCode: 0,
