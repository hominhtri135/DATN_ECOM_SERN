  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!id) {
        ...
        await blog.save();
        const res = await db.Blog.findOne({
          where: { id: id },
          include: [
            {
              model: db.Allcode,
...
const getAllBlog = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const res = await db.Blog.findAndCountAll(objectFilter);
      if (res.rows && res.rows.length > 0) {
        for (let i = 0; i < res.rows.length; i++) {
          res.rows[i].image = Buffer.from(res.rows[i].image, "base64").toString(
            "binary"
          );
...
const getFeatureBlog = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      const res = await db.Blog.findAll({
        where: { statusId: "S1" },
        include: [
          {
            model: db.Allcode,
