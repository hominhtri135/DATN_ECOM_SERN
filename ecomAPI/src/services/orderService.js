...
const { EXCHANGE_RATES } = require("../utils/constants");
    ...
    try {
      if (!data.addressUserId || !data.typeShipId) {
        ...
        const product = await db.OrderProduct.create({
          ...
          isPayment: data.isPaymentOnline,
          statusId: "S3",
...
const getAllOrders = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const res = await db.OrderProduct.findAndCountAll(objectFilter);
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.id || !data.statusId) {
        ...
        if (data.statusId === "S6") order.isPayment = 1;
        await order.save();
...
const getAllOrdersByShipper = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const res = await db.OrderProduct.findAll(objectFilter);
...
const paymentOrderVnpay = (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const returnUrl = process.env.URL_REACT + process.env.VNP_RETURNURL;
      ...
      if (bankCode !== null && bankCode !== "") {
        vnp_Params["vnp_BankCode"] = bankCode; //optional
      }
      ...
      vnp_Params["vnp_OrderType"] = orderType; //optional
...
const paymentOrderVnpaySuccess = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      const product = await db.OrderProduct.create({
        addressUserId: data.addressUserId,
