...
const createNewReceiptDetail = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (
        !data.receiptId ||
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!id) {
        ...
        const res = await db.Receipt.findOne({
          where: { id: id },
        });
...
const getAllReceipt = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      const objectFilter = { order: [["createdAt", "DESC"]] };
      ...
      const res = await db.Receipt.findAndCountAll(objectFilter);
