...
function InfoDetailProduct(props) {
  ...
  useEffect(() => {
    let dataProductDetail = dataProduct?.productDetail
      ? dataProduct?.productDetail
      : [];
    if (dataProductDetail.length > 0) {
      console.log(dataProductDetail, true);
      setproductDetail(dataProductDetail);

      setarrDetail(dataProductDetail[0]);
      setactiveLinkId(dataProductDetail[0]?.productDetailSize[0]?.id);
      setquantity(dataProductDetail[0]?.productDetailSize[0]?.stock);
      ...
      <div className="col-lg-5 offset-lg-1">
        <div className="s_product_text">
          ...
          <div className="card_area">
            <button className="main_btn" onClick={() => handleAddShopCart()}>
              Thêm vào giỏ
            </button>
          </div>
        </div>
